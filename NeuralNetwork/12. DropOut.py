...
class BatchNormalization(Layer):
    ...
    def forward(self, x):
        # 注意：严格的BN层也需要区分训练和推理。
        # 推理时应使用全局移动平均的均值和方差，这里为了简化，保持原样。
        self.x = x
...
class TwoLayerNetWithLayers:
    ...
    def numerical_gradient(self, x, t):
        # 此函数不再完整，因为Dropout的随机性会干扰数值梯度计算
        # 我们依赖反向传播，所以可以忽略此函数
        def loss_W(_):
    ...
    def gradient(self, x, t):
        ...
        # 反向遍历所有层
        # 【修改】反向传播的层列表需要与前向传播一致
        layers_to_backprop = list(self.layers)  # 创建一个副本
        ...
        grads = {
            ...
            "W2": self.layers[4].dW,  # 索引变为4
            "b2": self.layers[4].db,  # 索引变为4
            "gamma2": self.layers[5].dgamma,  # 索引变为5
            "beta2": self.layers[5].dbeta,  # 索引变为5
        }
...
if __name__ == "__main__":
    ...
    # 【修改】在初始化网络时传入dropout_ratio
    # 常见的dropout_ratio值为0.5
    ...
    for epoch in range(epoch_num):
        ...
        # 在计算整个数据集的准确率时，内部会调用 predict(train_flg=False)
